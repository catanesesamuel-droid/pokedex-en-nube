...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if not target_id:
        return error_response("user_id es obligatorio", 400)

    # No puede bloquearse a sí mismo
    if target_id == payload["sub"]:
        return error_response("No puedes bloquearte a ti mismo", 400)
    ...
    estado = "bloqueado" if not user["is_active"] else "activado"
    return success_response({"message": f"Usuario {estado} correctamente"})